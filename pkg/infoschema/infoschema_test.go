...
func TestConvertToTableSchemas(t *testing.T) {
	...
	m := map[string][]*model.TableInfo{
		"d1": {{
			...
			Columns: []*model.ColumnInfo{
				{Name: model.NewCIStr("c1"), FieldType: *(types.NewFieldType(mysql.TypeLong)), Comment: "foo"},
				{Name: model.NewCIStr("c2"), FieldType: *(types.NewFieldType(mysql.TypeString)), Comment: "bar"},
				{Name: model.NewCIStr("c3"), FieldType: *(types.NewFieldType(mysql.TypeLong)), Comment: "foobar"},
			},
		}},
		"d2": {{
			Name: model.NewCIStr("t1"),
	...
	a := require.New(t)
	back, err := infoschema.ConvertToTableSchemas(m)
	a.NoError(err)
	a.Equal(tables, back)
}
