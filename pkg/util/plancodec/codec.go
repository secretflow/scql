...
func DecodePlan(planString string) (string, error) {
	...
	pd, ok := decoderPool.Get().(*planDecoder)
	if !ok {
		return "", fmt.Errorf("assert failed while decodePlan, expected: *planDecoder, actual: %T", decoderPool.Get())
	}
...
func DecodeNormalizedPlan(planString string) (string, error) {
	...
	pd, ok := decoderPool.Get().(*planDecoder)
	if !ok {
		return "", fmt.Errorf("assert failed while decodeNormalizedPlan, expected: *planDecoder, actual: %T", decoderPool.Get())
	}
	defer decoderPool.Put(pd)
	pd.buf.Reset()
