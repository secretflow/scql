...
func NewIntraServer(app *application.App) (*http.Server, error) {
	...
	router.POST(constant.DeleteProjectPath, intraSvc.DeleteProjectHandler)
	...
	router.POST(constant.CreateViewPath, intraSvc.CreateViewHandler)
	router.POST(constant.ListViewsPath, intraSvc.ListViewsHandler)
	router.POST(constant.DropViewPath, intraSvc.DropViewHandler)
